...
export const getStudentProfileById = (userId) => async (dispatch) => {
  try {
    const res = await axios.get(`/profile/student/${userId}`);
...
export const getMentorProfileById = (userId) => async (dispatch) => {
  try {
    const res = await axios.get(`/profile/mentor/${userId}`);
...
export const getStudentProfiles = () => async (dispatch) => {
  dispatch({ type: CLEAR_PROFILE });
  try {
    const res = await axios.get('/profile/students');
...
export const getMentorProfiles = () => async (dispatch) => {
  dispatch({ type: CLEAR_PROFILE });
  try {
    const res = await axios.get('/profile/mentors');
...
export const createProfile =
  ...
  async (dispatch) => {
    try {
      ...
      const res = await axios.post('/profile/student', formData, config);
      dispatch(
...
export const createMentorProfile =
  ...
  async (dispatch) => {
    try {
      ...
      const res = await axios.post('/profile/mentor', formData, config);
      dispatch(
        setAlert(edit ? 'Profile Updated' : 'Profile Created', 'success')
      );
      if (!edit) {
...
export const addTimeSlot = (time) => async (dispatch) => {
  try {
    const config = {
      headers: {
        'Content-Type': 'application/json',
    ...
    const res = await axios.post('/profile/slots', time, config);
    dispatch(setAlert('Slot Added', 'success'));
    dispatch({
      type: UPDATE_PROFILE,
      payload: res.data,
    });
...
export const deleteSlot = (id) => async (dispatch) => {
  try {
    const res = await axios.delete(`/profile/slots/${id}`);
    dispatch({
      type: UPDATE_PROFILE,
      payload: res.data,
    });
    dispatch(setAlert('Slot Removed', 'success'));
  } catch (err) {
    dispatch({
      type: PROFILE_ERROR,
    });
